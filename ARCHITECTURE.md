...
| **"Is it behaving?" has no answer without measurement** | Red-team corpora measure detection against *attacks*; most real incidents are not attacks, and most sessions are not incidents at all. Measured on 1,730 real `gemini-3.7-flash` sessions: Layer 1 found **0** breaches against 299 refunds and 219 egress calls — a true zero, but only demonstrable because it was measured. | Ground truth defined over the **trajectory**, not the user (`v_session_labels`), so a policy breach in a benign session still counts — §6.5. The scripted suite carries labelled ground truth; the real corpus carries evidence of behaviour |
